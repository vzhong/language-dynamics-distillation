...
def nonzero_mean(emb):
    ...
    optional batch dimension. (H,W) is the map dim, L the layers and
...
class EMMA(nn.Module):
    ...
    def act(self, state, temb, memory, output_weights=False, return_state=False):
        '''
        temb: (num_sent, sent_len, emb_dim)
        '''
        ...
        if self.training:
            ...
            action = action
        if return_state:
            return action, state_emb
        else:
            return action

    def evaluate(self, state, action, temb):
        '''
        temb: (bs, sent_num, sent_len, emb_dim)
        '''
