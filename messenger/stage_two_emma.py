...
def train(args):
    ...
    if args.pretrain == 'initialize' or args.pretrain == 'distill' or args.pretrain == 'reward' or args.pretrain == 'noinit':
        from model.mymodel import Model as DynamicsModel
        dynamics_model = DynamicsModel(argparse.Namespace()).to(torch.device('cuda'))
        dynamics_model.load_from_checkpoint(checkpoint_path=args.pretrain_checkpoint)
        fdynamics = args.output + '_dynamics.pt'
        torch.save(dynamics_model.emma.state_dict(), fdynamics)
        dynamics_model.eval()
        if args.pretrain != 'reward' and args.pretrain != 'noinit':
            print('LOADING PRETRAIN {}'.format(args.pretrain))
            args.load_state = fdynamics
    elif args.pretrain == 'imitate':
        from model.imitate import Model as ImitateModel
        teacher = ImitateModel(argparse.Namespace(freverse_checkpoint='saves/reverse-default/last.ckpt'))
        teacher.load_from_checkpoint(checkpoint_path=args.pretrain_checkpoint)
        fsave = args.output + '_imitate.pt'
        torch.save(teacher.emma.state_dict(), fsave)
        del teacher
        args.load_state = fsave
    ...
    env = TwoEnvWrapper(2, 'train-sc', 'train-mc')
    eval_env = StageTwo(split='val')
    ...
    while True: # main training loop
        ...
        for t in range(args.max_steps):
            ...
            # Running policy_old:
            if args.pretrain == 'distill' or args.pretrain == 'noinit':
                obs = buffer.get_obs()
                action, state_emb = ppo.policy_old.act(obs, text, memory, return_state=True)
                with torch.no_grad():
                    _, dynamics_state = dynamics_model.emma.act(obs, text, memory, return_state=True)
                if args.pretrain == 'distill' or args.pretrain == 'noinit':
                    distill_diffs.append(((state_emb - dynamics_state.detach())**2).mean())
            elif args.pretrain == 'reward':
                obs = buffer.get_obs()
                action = ppo.policy_old.act(obs, text, memory)
                with torch.no_grad():
                    dynamics_pred = dynamics_model.one_step(obs, text).max(-1)[1].detach().cpu()
            else:
                action = ppo.policy_old.act(buffer.get_obs(), text, memory)
            state, reward, done, _ = env.step(action)

            aux_reward = 0
            if args.pretrain == 'reward':
                match = torch.from_numpy(state['entities']).eq(dynamics_pred)
                aux_reward = 0.01 * match.float().mean().item()
            ...
            memory.rewards.append(reward + aux_reward)
            memory.is_terminals.append(done)

            # update the model if its time
            if timestep % args.update_timestep == 0:
                if args.pretrain == 'distill' or args.pretrain == 'noinit':
                    ppo.update(memory, distill=distill_diffs)
                else:
                    ppo.update(memory)
                memory.clear_memory()
                timestep = 0
...
if __name__ == "__main__":
    ...
    parser.add_argument("--load_imitate_state", default=None, help="Path to model state dict.")
    ...
    parser.add_argument("--max_eps", default=1e6, type=float, help="max training episodes")
    ...
    parser.add_argument("--pretrain", default='rand', choices=('rand', 'initialize', 'distill', 'reward', 'imitate', 'noinit'), help="Use teacher as reward.")
    parser.add_argument("--pretrain_checkpoint", help="Use teacher as reward.")
