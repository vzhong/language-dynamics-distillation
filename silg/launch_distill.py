...
parser.add_argument('--envs', nargs='+', default=['rtfm', 'alfworld', 'touchdown_lang'], help='environments to run')
parser.add_argument('--partition', help='slurm partition', default='learnfair')
...
if __name__ == '__main__':
    ...
    f = 'alfworld/last.ckpt'
    opts_list['alfworld'] = get_opts(
        model=['multi_rank'],
        entropy_cost=[0.005],
        stateful=[False],
        use_local_conv=[False],
    ...
    f = 'touchdown_lang/last.ckpt'
    opts_list['touchdown_lang'] = get_opts(
        model=['multi_nav_emb'],
        entropy_cost=[0.05],
        ...
        unroll_length=[64],
        num_film=[3],
        env=['silg:td_segs_lang_train-v0'],
        val_env=['silg:td_segs_lang_dev-v0'],
        pretrain=[f],
        teacher=[f],
    )
            ...
            for seed in range(args.seeds):
                ...
                if not args.dry:
                    ...
                    if args.local:
                        ...
                        slurm_job = executor.submit(run_exp.main, flags)
                n += 1
