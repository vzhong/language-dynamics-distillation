...
def learn(actor_model,
    ...
    with lock:
        learner_outputs, unused_state = model(batch, initial_agent_state, teacher_model=teacher_model)
        ...
        if flags.reward_teacher:
            rewards = rewards - 0.01 * learner_outputs['aux_loss'].mean().item()
        ...
        aux_loss = learner_outputs['aux_loss'][0] if flags.distill_teacher else torch.tensor([0], dtype=torch.float, device=pg_loss.device)
...
class Train(SlurmJob):
    ...
    def load_state_dict(self, d):
        ...
        self.learner_model.load_state_dict(d['model_state_dict'], strict=False)
        self.optimizer.load_state_dict(d['optimizer_state_dict'], strict=False)
        self.scheduler.load_state_dict(d['scheduler_state_dict'], strict=False)
    ...
    def initialize_learner_model(self):
        ...
        if self.learner_model is None:
            ...
            if flags.pretrain is not None:
                print('Loading pretrained learner model from {}'.format(flags.pretrain))
                checkpoint = torch.load(flags.pretrain)['state_dict']
                state_dict = {k.replace('inner_model.', ''): v for k, v in checkpoint.items() if 'inner_model' in k}
                if not flags.reward_teacher:
                    self.learner_model.load_state_dict(state_dict, strict=False)

        self.teacher_model = None
        if flags.teacher is not None:
            self.teacher_model = Net.make(flags, env).to(device=get_device(flags))
            checkpoint = torch.load(flags.teacher)['state_dict']
            state_dict = {k.replace('inner_model.', ''): v for k, v in checkpoint.items() if 'inner_model' in k}
    ...
    def forward(self, exp_path):
        ...
        if flags.wandb:
            ...
            self.exp.loggers.append(WandbLogger(project=os.path.basename(flags.savedir), name=flags.xpid.replace(':', '_'), entity=os.environ['USER']))
        ...
        def batch_and_learn(i, lock=threading.Lock()):
            ...
            while self.frames < flags.total_frames:
                ...
                stats = learn(model, learner_model, batch, agent_state, optimizer, scheduler, flags, teacher_model=teacher_model)
...
def rollout(flags, verbose=False, num_eps=None):
    num_eps = num_eps or flags.eval_eps  # number of eps to run for
    Net = importlib.import_module('model.{}'.format(flags.model)).Model
    gym_env = Net.create_env(flags)
    env = environment.Environment(gym_env)
    ...
    while len(won) < num_eps:
        ...
        while not done:
            agent_outputs, agent_state = model(observation, agent_state)
            to = observation.copy()
            to['action'] = agent_outputs['action']
            traj.append(to)
            ...
            if observation['done'].item() or len(traj) > 200:
                returns.append(observation['episode_return'].item())
                won.append(observation['reward'][0][0].item() > 0.5)
                ep_len.append(steps)
                agent_state = model.initial_state(batch_size=1)
...
def main(flags, **kwargs):
    if flags.mode == 'train':
        ...
        job = Train()
        job(exp.explog)
    elif flags.mode == 'test' or flags.mode == 'test_render':
        ...
        assert flags.resume
        fout = flags.resume.replace('job.tar', 'rollout.pt')
        assert fout != flags.resume
        with open(flags.resume.replace('job.tar', 'exp.json')) as f:
            exp = json.load(f)
        old_flags = argparse.Namespace(**exp['config'])
        old_flags.resume = flags.resume
        rollouts, stats = rollout(old_flags, verbose=True, num_eps=5000)
        with open(fout + '.json', 'wt') as f:
            json.dump(stats, f, indent=2)
        torch.save(rollouts, fout)
