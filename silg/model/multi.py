...
class Model(nn.Module):
    ...
    def forward(self, inputs, core_state, return_intermediate=False, teacher_model=None):
        if teacher_model is not None:
            with torch.no_grad():
                teacher_ret, teacher_core_state = teacher_model.forward(inputs, core_state, return_intermediate=True)
        ...
        if self.training and not return_intermediate:
        ...
        if teacher_model is not None:
            diff = rep - teacher_ret['rep']
            diff_squared = diff ** 2
            aux_loss = torch.mean(diff_squared.view(T, -1), dim=1)
        else:
            aux_loss = torch.Tensor([0] * T).to(cell.device)

        ret = dict(policy_logits=policy_logits, baseline=baseline, action=action, aux_loss=aux_loss)
        if return_intermediate:
            ret.update(dict(rep=rep, conv=conv))
        return ret, core_state
